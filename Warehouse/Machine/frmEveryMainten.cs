...
{
    public partial class frmEveryMainten : Form
    {
        public frmEveryMainten()
        {
            InitializeComponent();
            LoadControl();
        }
        ...
        void LoadControl()
        {
            LoadStatus();
            LoadData();
            Group.Text = ("danh sách hạng mục cần kiểm tra, máy " + Kun_Static.MachineCode).ToUpper();
        }
        void LoadStatus()
        {
            ...
            gridView1.Columns["Result"].ColumnEdit = _riEditor;
        }
        void LoadData()
        {
            ...
            List<HistoryDeviceDTO> listH = MachineDAO.Instance.GetListHistoryDeviceLong(date1, date2, MachineCode);
            List<CheckDeviceDTO> listC = MachineDAO.Instance.GetListEveryMaintenBy(MachineCode);
            if (listC.Count == 0)
            {
                dk = 1;
                listC = MachineDAO.Instance.GetListEveryMainten(MachineCode);
            }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (LoadCondition())
            {
                string Employees = Kun_Static.accountDTO.UserName;
                ...
                try
                {
                    ...
                    foreach (var item in gridView1.GetSelectedRows())
                    {
                        ...
                        if (CheckTest == -1)
                        {
                            MachineDAO.Instance.InsertHistoryDevice(IdRelationShip, count, statusHD, today, Employees.ToUpper(), Note, status);
        ...
        private void frmEveryMainten_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void gridView1_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            ...
            if (dk == 0)
            {
                if (e.RowHandle >= 0)
                {
                    ...
                    string MachineCode = Kun_Static.MachineCode;
                    List<HistoryDeviceDTO> listH = MachineDAO.Instance.GetListHistoryDeviceLong(date1, date2, MachineCode);
                    int Id = int.Parse(view.GetRowCellValue(e.RowHandle, view.Columns["IdRelationShip"]).ToString());
                    if (listH.Count > 0)
                    {
                        MachineDAO.Instance.UpdateStatusRelationShip(Id, 0);
                    }
                    else
                    {
                        int CheckTest = MachineDAO.Instance.TestCheckHistory(Id, date1, date2);
                        if (CheckTest == 1)
                        {
                            MachineDAO.Instance.UpdateStatusRelationShip(Id, 0);
                        }
                        int Status = MachineDAO.Instance.StatusRelationShip(Id);
                        switch (Status)
                        {
                            case 1:
            ...
            try
            {
                ...
                if (e.Column.Name == "colReality")
                {
                    ...
                    if (confirm == 1)
                    {
                        ...
                        float Limit = (float)Convert.ToDouble(view.GetRowCellValue(e.RowHandle, view.Columns["Limit"]).ToString());
                        float Detail = (float)Convert.ToDouble(view.GetRowCellValue(e.RowHandle, view.Columns["Detail"]).ToString());
                        float abs = (float)Math.Round(Math.Abs(Detail - Reality), 2);
                        if (abs > Limit)
                        {
        ...
        private void btnMethod_Click(object sender, EventArgs e)
        {
            ...
            string url = MachineDAO.Instance.GetItemDevice(id).UrlEveryMainten;
            if (url.Length > 0)
            {
