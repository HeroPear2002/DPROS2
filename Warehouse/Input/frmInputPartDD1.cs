...
{
    public partial class frmInputPartDD1 : Form
    {
        public frmInputPartDD1()
        {
            InitializeComponent();
            LoadControl();
        }
        ...
        void LoadUpdate()
        {
            ...
            foreach (IvenstoryDTO item in listI1)
            {
                ...
                if (iventory > 0)
                {
                    ...
                    EditHistoryDAO.Instance.Insert(DateTime.Now, Kun_Static.EmployessCode, "chuyển Id Input " + item.Id +" về 0 , Update vị trí về 4 , Xóa vị trí còn lại", item.Name);
                }
            }
        }
        #endregion
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (barCode.Length != 0)
            {
              ...
              if(leng == 5)
                ...
                {
                    MessageBox.Show("bạn bấm sai mác sản phẩm hãy kiểm tra lại".ToUpper(),"Thông Báo",MessageBoxButtons.OK,MessageBoxIcon.Error);
                    return;
                }
                #endregion
                ...
                if (quantityInput > 0)
                {
                    if (MessageBox.Show(msg.ToUpper(), "Thông Báo", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        ...
                        if (test == -1)
                        {
                            IventoryPartDAO.Instance.InputPart(partCode, dateInput, quantityInput, employess, molNumber, machineCode, idWarehouse, dateManufacturi, 0, "Nhập chuyển kho", lot, notePC, notePC,factoryCode);
                            WareHouseDAO.Instance.UpdateStatusWH(idWarehouse,10);
                        }
